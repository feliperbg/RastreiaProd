document.addEventListener('DOMContentLoaded', function() {
    ...
    function createFloatingScrollbar() {
        ...
        console.log('Criando barra de rolagem flutuante...');
        ...
        console.log('Largura do conteúdo:', tableWrapper.scrollWidth, 'Largura visível:', tableWrapper.clientWidth);
        
        if (!needsScrollbar) {
            console.log('Scrollbar não necessária');
            return;
        }
        ...
        // Atualiza dimensões e mostra a scrollbar
        updateScrollbarDimensions();
        checkScrollbarVisibility();
    }

    function updateScrollbarDimensions() {
        if (!floatingScrollbarWrapper || !floatingScrollbarInner) {
            return;
        }
        ...
        if (!needsScrollbar) {
            floatingScrollbarWrapper.style.display = 'none';
            return;
        }
        
        const tableRect = tableWrapper.getBoundingClientRect();
        
        // Atualiza posição e dimensões
        floatingScrollbarWrapper.style.left = tableRect.left + 'px';
        floatingScrollbarWrapper.style.width = tableWrapper.clientWidth + 'px';
        floatingScrollbarWrapper.style.right = 'auto';
        floatingScrollbarWrapper.style.display = 'block';
        floatingScrollbarInner.style.width = tableWrapper.scrollWidth + 'px';
        
        console.log('Dimensões atualizadas - Left:', tableRect.left, 'Width:', tableWrapper.clientWidth);
    }
    ...
    function checkScrollbarVisibility() {
        ...
        // Mostra se a tabela está pelo menos parcialmente visível
        if (tableRect.bottom > 0 && tableRect.top < viewportHeight) {
            const needsScrollbar = tableWrapper.scrollWidth > tableWrapper.clientWidth;
            if (needsScrollbar) {
                floatingScrollbarWrapper.style.visibility = 'visible';
                floatingScrollbarWrapper.style.display = 'block';
                console.log('Scrollbar VISÍVEL');
            }
        } else {
            floatingScrollbarWrapper.style.visibility = 'hidden';
            console.log('Scrollbar ESCONDIDA - tabela fora da viewport');
        }
    }

    // Função unificada para atualizar com debounce
    ...
    // MutationObserver para mudanças de classe no body (sidebar toggle)
    ...
    // Observer para mudanças no conteúdo da tabela
    const observerConteudoTabela = new MutationObserver(() => {
        setTimeout(() => {
    ...
    if (tabelaBody) {
        observerConteudoTabela.observe(tabelaBody, { 
            childList: true
        });
    }

    // Configuração inicial - aguarda carregamento completo
    setTimeout(() => {
        createFloatingScrollbar();
    }, 500);
});
